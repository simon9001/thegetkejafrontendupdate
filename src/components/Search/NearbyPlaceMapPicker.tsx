...
const pinIcon = L.divIcon({
  ...
  html: `<div style="width:28px;height:28px;border-radius:50% 50% 50% 0;
    background:#DD6E42;border:2px solid white;transform:rotate(-45deg);
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#50757A] pointer-events-none"/>
        <input
          type="text"
          value={searchText}
          ...
          className="w-full pl-9 pr-8 py-2.5 border border-[#EAEAEA] rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#DD6E42]/20 focus:border-[#DD6E42]"
        />
        {searching && (
          <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#50757A] animate-spin"/>
        )}
        {!searching && searchText && (
          <button type="button" onClick={clearSearch}
            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#50757A] hover:text-[#222]">
        ...
        {showResults && results.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-[#EAEAEA] rounded-xl shadow-lg z-[2000] max-h-52 overflow-y-auto">
            {results.map((r) => (
              <button
                key={r.place_id}
                type="button"
                onClick={() => handleSelectResult(r)}
                className="w-full flex items-start gap-2.5 px-4 py-2.5 hover:bg-[#fff0f2] text-left transition-colors border-b border-[#EAEAEA] last:border-0"
              >
                <MapPin className="w-3.5 h-3.5 text-[#DD6E42] mt-0.5 shrink-0"/>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-[#222] truncate">{r.display_name.split(',')[0]}</p>
                  <p className="text-[11px] text-[#50757A] truncate">{r.display_name.split(',').slice(1,3).join(',')}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {/* Leaflet map */}
      <div className="rounded-xl overflow-hidden border border-[#EAEAEA]" style={{ height: 300 }}>
        <MapContainer
          center={center}
          zoom={13}
      ...
      {/* Coordinates readout */}
      {hasPin ? (
        <p className="text-[11px] text-[#50757A] flex items-center gap-1">
        ...
        <p className="text-[11px] text-[#50757A] italic">No pin yet — click the map or search above.</p>
      )}
    </div>
  );
};
