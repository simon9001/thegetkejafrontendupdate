          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-[#50757A] flex items-center justify-center shrink-0">
                <span className="text-[#DD6E42] font-bold">{c.name[0]}</span>
              </div>
              <div>
                <h3 className="font-bold text-[#50757A]">{c.name}</h3>
                <p className="text-xs text-[#50757A]">{c.property}</p>
              </div>
            </div>
            <Badge status={c.status} />
          </div>
          <div className="space-y-1.5 mb-4">
            <p className="text-xs text-[#50757A] flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" />{c.phone}</p>
            <p className="text-xs text-[#50757A] flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" />{c.name.toLowerCase().replace(' ', '.')}@example.com</p>
          </div>
          <div className="flex gap-2">
            <button className="flex-1 px-3 py-2 border border-gray-200 rounded-xl text-xs font-bold text-[#50757A] hover:bg-gray-50 transition-colors">Message</button>
            <button className="w-9 h-9 flex items-center justify-center border border-gray-200 rounded-xl text-[#50757A] hover:bg-gray-50 transition-colors">
