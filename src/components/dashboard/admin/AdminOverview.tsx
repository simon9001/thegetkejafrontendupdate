  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs">
        <span className="text-[#50757A] font-medium">{label}</span>
        <span className="text-[#50757A] font-semibold">KES {value.toLocaleString()}</span>
      </div>
...
const QueueRow: React.FC<{
  ...
  return (
    <button onClick={onClick} className="w-full flex items-center justify-between py-3 px-4 rounded-xl hover:bg-gray-50 transition-colors group">
      <span className="text-sm font-medium text-[#50757A]">{label}</span>
      <div className="flex items-center gap-2">
        <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${colors[urgency]}`}>{count}</span>
        <ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-[#50757A] transition-colors" />
      </div>
    </button>
  );
};
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Revenue Breakdown" sub="Current month, all streams" action={
            <button onClick={() => onNavigate('revenue')} className="text-xs text-[#DD6E42] font-semibold hover:underline flex items-center gap-1">
          ...
          {revenueLoading ? (
            ...
            <div className="space-y-4">
              <RevenueBar label="Listing Fees" value={revenue?.by_stream?.listing_fees_kes ?? 0} total={revenueTotal} color="bg-[#DD6E42]" />
              ...
              <div className="pt-2 border-t border-gray-100 flex justify-between text-sm">
                <span className="text-[#50757A] font-medium">Total this period</span>
                <span className="text-[#50757A] font-bold">KES {revenueTotal.toLocaleString()}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Moderation Queue" sub="Items requiring admin action" action={
            <button onClick={() => onNavigate('moderation')} className="text-xs text-[#DD6E42] font-semibold hover:underline flex items-center gap-1">
              View all <ChevronRight className="w-3.5 h-3.5" />
            </button>
          } />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Users by Role" action={<button onClick={() => onNavigate('users')} className="text-xs text-[#DD6E42] font-semibold hover:underline">View →</button>} />
          <div className="space-y-2.5">
            {Object.entries(userStats?.by_role ?? {}).map(([role, count]) => (
              <div key={role} className="flex items-center justify-between text-sm">
                <span className="text-[#50757A] capitalize font-medium">{role.replace('_', ' ')}</span>
                <span className="font-bold text-[#50757A]">{fmt(count as number)}</span>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Properties" action={<button onClick={() => onNavigate('properties')} className="text-xs text-[#DD6E42] font-semibold hover:underline">View →</button>} />
          <div className="space-y-2.5">
            ...
            <div className="pt-2 border-t border-gray-100 flex justify-between text-sm">
              <span className="text-[#50757A] font-medium">Avg search score</span>
              <span className="font-bold text-[#50757A]">{propStats?.avg_search_score ?? '—'}</span>
            </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Bookings" action={<button onClick={() => onNavigate('bookings')} className="text-xs text-[#DD6E42] font-semibold hover:underline">View →</button>} />
          <div className="space-y-2.5">
            <div className="flex justify-between text-sm"><span className="text-[#50757A] font-medium">Short-stay active</span><span className="font-bold text-[#50757A]">{fmt(bookingStats?.short_stay?.active)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#50757A] font-medium">Escrow held</span><span className="font-bold text-emerald-600">KES {fmt(bookingStats?.short_stay?.escrow_held_kes)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#50757A] font-medium">Disputes</span><span className="font-bold text-red-500">{fmt(bookingStats?.short_stay?.disputed)}</span></div>
            <div className="pt-2 border-t border-gray-100 flex justify-between text-sm"><span className="text-[#50757A] font-medium">Long-term active</span><span className="font-bold text-[#50757A]">{fmt(bookingStats?.long_term?.active)}</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#50757A] font-medium">Visits pending</span><span className="font-bold text-amber-600">{fmt(bookingStats?.visits?.pending_confirmation)}</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Reviews" action={<button onClick={() => onNavigate('reviews')} className="text-xs text-[#DD6E42] font-semibold hover:underline">View →</button>} />
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[
              ...
              <div key={label} className={`${bg} rounded-xl p-3 text-center`}>
                ...
                <p className="text-[10px] text-[#50757A] font-medium">{label}</p>
              </div>
            ))}
          </div>
          <div className="flex justify-between text-sm border-t border-gray-100 pt-3">
            <span className="text-[#50757A] font-medium">Avg rating</span>
            <span className="font-bold text-[#50757A] flex items-center gap-1">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          <SectionHeader title="Subscriptions" action={<button onClick={() => onNavigate('subscriptions')} className="text-xs text-[#DD6E42] font-semibold hover:underline">View →</button>} />
          <div className="space-y-2.5">
            {Object.entries(subStats?.subscribers_by_plan ?? {}).map(([plan, count]) => (
              <div key={plan} className="flex items-center justify-between text-sm">
                <span className="text-[#50757A] font-medium">{plan}</span>
                <div className="flex items-center gap-2">
                  <div className="w-20 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#DD6E42] rounded-full" style={{ width: `${subStats ? ((count as number) / subStats.total) * 100 : 0}%` }} />
                  </div>
                  <span className="font-bold text-[#50757A] w-8 text-right">{fmt(count as number)}</span>
                </div>
              </div>
            ))}
            <div className="pt-2 border-t border-gray-100 flex justify-between text-sm">
              <span className="text-[#50757A] font-medium">Past due</span>
