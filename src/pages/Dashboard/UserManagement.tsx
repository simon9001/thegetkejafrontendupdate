    ...
    return (
        <DashboardLayout>
            <div className="space-y-8 pb-12">
                {/* Header Section */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-[#50757A] p-6 md:p-8 rounded-[32px] border border-[#50757A] relative overflow-hidden">
                    <div className="relative z-10">
                        <h1 className="text-3xl font-black text-white">User Intelligence</h1>
                        <p className="text-[#DD6E42] font-bold mt-1 tracking-tight opacity-80 uppercase text-xs">Manage platform access and permissions</p>
                    </div>
                    ...
                    <div className="relative w-full md:w-96 z-10">
                        ...
                        <input
                            ...
                            className="w-full bg-[#50757A] border border-[#50757A] rounded-2xl py-3 pl-12 pr-4 text-sm text-white focus:ring-2 focus:ring-[#DD6E42]/30 outline-none transition-all placeholder:text-gray-600 font-medium"
                        />
                    </div>
                    {/* Decorative elements */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-[#DD6E42]/5 rounded-full -mr-32 -mt-32 blur-3xl"></div>
                </div>

                {/* Table Container */}
                <div className="bg-[#50757A] rounded-[32px] border border-[#50757A] overflow-hidden shadow-2xl shadow-black/20">
                                ...
                                {isLoading ? (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-20 text-center">
                                            <div className="animate-spin rounded-full h-8 w-8 border-2 border-[#DD6E42] border-t-transparent mx-auto"></div>
                                        </td>
                                    </tr>
                                    ...
                                    <tr key={user.id} className="hover:bg-white/[0.02] transition-colors group">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-xl bg-[#50757A] flex items-center justify-center text-[#DD6E42] border border-[#50757A] font-black uppercase text-xs">
                                                    {user.avatar_url ? (
                                                        <img src={user.avatar_url} alt="" className="w-full h-full object-cover rounded-xl" />
                                                    ) : user.full_name.charAt(0)}
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-white group-hover:text-[#DD6E42] transition-colors">{user.full_name}</p>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex flex-wrap gap-1.5">
                                                {user.roles.map((role: string) => (
                                                    <span key={role} className="px-2 py-0.5 bg-[#DD6E42]/10 text-[#DD6E42] text-[10px] font-black uppercase tracking-tighter rounded-md border border-[#DD6E42]/20">
                                                        {role}
                                                    </span>
                                                ))}
                                            </div>
                                        ...
                                        <td className="px-6 py-5">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${user.status === 'active' ? 'bg-green-500/10 text-green-500' :
                                                user.status === 'pending' ? 'bg-[#E8DAB2]/10 text-[#E8DAB2]' :
                                        ...
                                        <td className="px-6 py-5 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    ...
                                                    className="p-2 bg-[#50757A] text-gray-400 hover:text-[#DD6E42] rounded-lg border border-[#50757A] transition-all"
                                                    title="Change Role"
                                                >
                                                    <UserCog size={16} />
                                                </button>
                    ...
                    {/* Pagination */}
                    <div className="px-6 py-5 flex items-center justify-between bg-[#50757A]/50 border-t border-[#50757A]">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                disabled={page === 1}
                                className="p-2 bg-[#50757A] text-gray-400 hover:text-white rounded-lg border border-[#50757A] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                            >
                                <ChevronLeft size={16} />
                            </button>
